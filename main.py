...
import pygame
from config import (
    SCREEN_WIDTH, SCREEN_HEIGHT, FPS, AI_DELAY, PEEK_REVEAL_SECONDS,
    ...
    while running:
                                    ...
                                    for slot_idx, rect in enumerate(rects):
                                        if rect.collidepoint(mouse_pos) and game_manager.players[human_idx].hand[slot_idx] is not None:
                                            ...
                                            if slot_idx < len(card_rects):
                                                ...
                                                renderer.set_peek_reveal(peeked_card, r.x, r.y, PEEK_REVEAL_SECONDS)
                                            awaiting = None
                                            status_message = ""
                                            turn_end_timer = 0.5
                                            break
        ...
        if current_screen == "game" and game_manager is not None:
            ...
            if not cp.is_human and turn_end_timer <= 0:
                ...
                if ai_phase == 'drawing' and ai_timer >= AI_DELAY:
